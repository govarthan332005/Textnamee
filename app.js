...
(async function() {
  ...
  function buildShareInviteLink() {
    ...
    var base = (runtimeLinksConfig.shareBaseLink || 'https://sliceinvest.app/signup').trim();
    if (!code) return base;
  ...
  function buildShareMessage() {
    ...
    var template = runtimeLinksConfig.shareMessageTemplate || "🚀 Join me on SliceInvest!\n\nUse my referral code *{CODE}* and start earning daily returns.\n\n🔗 {LINK}";
    return template
      .replace(/\{CODE\}/g, code || '—')
      .replace(/\{LINK\}/g, link)
  ...
  window.shareVia = function(channel) {
    ...
    var link = buildShareInviteLink();
    var title = runtimeLinksConfig.shareTitle || 'Join SliceInvest';
