...
(async function() {
  ...
  function startUserDocListener() {
    ...
    try {
      ...
      _userDocUnsub = onSnapshot(ref, function(snap) {
        ...
        if (elAP)   elAP.textContent   = fresh.activePlans || 0;
        if (elWB)   elWB.textContent   = fmt(fresh.withdrawableBalance || 0);
  ...
  window.updateWithdrawLivePreview = function() {
    ...
    var feeRate = 3; // 3% default — mirrors the static info-note text
